...
export const LeaderboardCard = ({ item, isCompact = false }: LeaderboardCardProps) => {
  ...
  const getRankColor = (rank: number) => {
    ...
    return "bg-white border-black shadow-[2px_2px_0px_black]";
  };
  ...
  if (isCompact) {
    return (
      <div className={cn(
        ...
        // Thêm height auto để card co giãn theo nội dung
        "h-auto min-h-[60px]"
      )}>
        {/* Rank Number */}
        ...
        {/* Name & School Info - ĐÃ SỬA Ở ĐÂY */}
        <div className="flex-1 min-w-0 flex flex-col justify-center">
          <p
            className="font-bold text-xs sm:text-sm text-black line-clamp-2 break-words leading-tight"
            title={item.userName}
          >
            {item.userName}
          </p>
          {(item.schoolName || item.className) && (
            <div className="flex items-start gap-1 mt-0.5">
              <GraduationCap size={10} className="text-slate-400 flex-shrink-0 sm:w-3 sm:h-3 mt-0.5" />
              <p
                className="text-[9px] sm:text-[10px] font-semibold text-slate-500 line-clamp-1 break-all"
                title={item.schoolName && item.className ? `${item.schoolName} - ${item.className}` : item.schoolName || item.className || undefined}
              >
                {item.schoolName && item.className
                  ? `${item.schoolName} - ${item.className}`
                  : item.schoolName || item.className}
  ...
  return (
    ...
    >
      <div className="flex items-start gap-2 sm:gap-3 md:gap-4 h-full">
        ...
        <div
          className={cn(
            "flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 md:w-16 md:h-16 rounded-lg sm:rounded-xl border-2 border-black flex items-center justify-center mt-1",
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-between h-full">
          {/* User Info - ĐÃ SỬA Ở ĐÂY */}
          <div className="flex items-start gap-2 sm:gap-3 mb-1.5 sm:mb-2">
            <div className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full border-2 border-black bg-blue-500 flex items-center justify-center text-white font-black overflow-hidden flex-shrink-0 text-xs sm:text-sm md:text-base mt-1">
              {item.userPicture ? (
                <img src={item.userPicture} alt={item.userName} className="w-full h-full object-cover" />
              ) : (
            ...
            <div className="min-w-0 flex-1">
              {/* Thay truncate bằng line-clamp-2 và break-words */}
              <p
                className="font-black text-sm sm:text-base md:text-lg text-black line-clamp-2 break-words leading-tight"
                title={item.userName}
              >
                {item.userName}
              </p>
              {(item.schoolName || item.className) && (
                <div className="flex items-start gap-1 mt-0.5">
                  <GraduationCap size={10} className="text-slate-500 flex-shrink-0 sm:w-3 sm:h-3 mt-[3px]" />
                  {/* Cho phép trường học xuống dòng nếu cần */}
                  <p
                    className="text-[9px] sm:text-[10px] font-bold text-slate-600 line-clamp-2 break-words leading-tight"
                    title={
                      item.schoolName && item.className
                        ? `${item.schoolName} - ${item.className}`
              ...
              <p className="text-[10px] sm:text-xs font-bold text-black/60 mt-1">
          ...
          <div className="flex justify-between items-end">
            ...
            <div className="flex flex-col items-end">
              ...
              <div className="flex items-center gap-0.5 sm:gap-1">
                ...
                <span className="font-bold text-xs sm:text-sm leading-none">{formatTime(item.durationSeconds)}</span>
              </div>
            </div>
          </div>
        </div>
